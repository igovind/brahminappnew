      ...
      onPressed: () {
        if (_formKeyE.currentState!.validate() &&
          ...
          _getLocation().whenComplete(() async {
            ...
            UploadTask uploadTask = reference.putFile(profilePicFile!);
            var downloadUrl =
           ...
           UploadTask uploadTask = reference.putFile(coverPicFile!);
            var downloadUrl =
                await (await uploadTask).ref.getDownloadURL();
            var url = downloadUrl.toString();
            coverPicUrl = url;
      ...
      child: Container(
        ...
        child: StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              List<dynamic> statesArray = snapshot.data!.get('states');
      ...
      child: Container(
        ...
        child: StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshot1) {
              ...
              List<dynamic> typeArray = snapshot1.data!.get('punditTypes');
        ...
        body: StreamBuilder<DocumentSnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.data!.data() != null) {
              pujaPicUrl = snapshot.data!.get('pujaPicUrl');
              fName = snapshot.data!.get('firstName');
              lName = snapshot.data!.get('lastName');
              aboutYou = snapshot.data!.get('aboutYou');
              punditType = snapshot.data!.get('type');
              state = snapshot.data!.get('state');
              profilePicUrl = snapshot.data!.get('profilePicUrl');
              coverPicUrl = snapshot.data!.get('coverpic');
              verified = snapshot.data!.get('verified');
              number = snapshot.data!.get('number');
            }
            return loading
                ? Center(
                    child: CircularProgressIndicator(),
