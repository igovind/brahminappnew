...
class _PujaEditTileState extends State<PujaEditTile> {
  ...
  void initState() {
    _price = "${widget.price}";
    widget.mainSamagriList!.forEach((element) {
      samagriItems.add(DropdownMenuItem(
          value:
              "${Language(code: widget.languageCode, text: element["name"]).getText}",
          child: Text(
              "${Language(code: widget.languageCode, text: element["name"]).getText}")));
      quantityIdMap.addAll({element["sid"]: ""});
    });
    ...
    if (_aFormKey.currentState!.validate()) {
      ...
      try {
        ...
        FireStoreDatabase(uid: widget.uid).setPujaOffering(data: {
          ...
          FirebaseFirestore.instance.doc("inventories/assignedPuja").update({
            "${widget.pjid}":FieldValue.arrayUnion([widget.uid])
          });
        }).whenComplete(() {
          BotToast.showText(
                              ...
                              Container(
                                  ...
                                  child: ListView.separated(
                                      ...
                                      itemBuilder: (context, index) => Row(
                                            ...
                                            children: [
                                              Text(
                                                  "${Language(code: widget.languageCode, text: widget.mainSamagriList![selectedSamagriIndex[index]]["name"]).getText}"),
                                              Container(
                                                ...
                                                child: TextFormField(
                                                  initialValue:
                                                      "${quantityIdMap[widget.mainSamagriList![selectedSamagriIndex[index]]["sid"]] == null ? "" : quantityIdMap[widget.mainSamagriList![selectedSamagriIndex[index]]["sid"]]}",
                                                  cursorColor:
                                                      Colors.deepOrange,
                                                  decoration: InputDecoration(
                                                    border: InputBorder.none,
                                                  ...
                                                  onChanged: (value) {
                                                 /*setState(() {
                                                   quantityIdMap.addAll({
                                                     quantityIdMap[widget
                                                         .mainSamagriList![
                                                     selectedSamagriIndex[
                                                     index]]["sid"]]: value
                                                   });*/
                                                   setState(() {
                                                     quantityIdMap[widget
                        ...
                        child: Container(
                          ...
                          child: ListView.builder(
                              ...
                              itemBuilder: (context, index) => Text(
                                  "${index + 1}-  ${Language(code: widget.languageCode, text: widget.mainSamagriList![selectedSamagriIndex[index]]["name"]).getText} ---- ${quantityIdMap[widget.mainSamagriList![selectedSamagriIndex[index]]["sid"]] == null ? "" : quantityIdMap[widget.mainSamagriList![selectedSamagriIndex[index]]["sid"]]} ")),
                        )),
                Align(
